...
def main():
    ...
    if args.save_csv:
        ...
        with open(args.save_csv, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['key', 'category', 'model_id', 'cd_pred', 'cd_noisy', 'cd_score',
                              'p2s_pred', 'p2s_noisy', 'p2s_score'])
            for key, cd_pred, cd_noisy, cd_s, p2s_pred, p2s_noisy, p2s_s in results:
                parts = key.split(os.sep)
                category = parts[0] if len(parts) > 0 else ''
                model_id = parts[1] if len(parts) > 1 else key
                writer.writerow([key, category, model_id, cd_pred, cd_noisy, cd_s,
                                  p2s_pred if p2s_pred is not None else '',
                                  p2s_noisy if p2s_noisy is not None else '',
                                  p2s_s if p2s_s is not None else ''])
            for key in missing_pred:
                parts = key.split(os.sep)
                category = parts[0] if len(parts) > 0 else ''
                model_id = parts[1] if len(parts) > 1 else key
                writer.writerow([key, category, model_id, '', '', 0.0, '', '', 0.0 if use_p2s else ''])
        print(f"逐样本得分已保存到: {args.save_csv}")
